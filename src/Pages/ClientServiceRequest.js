  ...
  useEffect(() => {
    ...
    // ✅ IF NO DATE SELECTED → TODAY FIRST
      ...
      data = [...data].sort((a, b) => {
        ...
        const aDateObj = safeDate(a.preferred_date);
        const bDateObj = safeDate(b.preferred_date);

        if (!aDateObj && !bDateObj) return 0;
        if (!aDateObj) return 1;   // push invalid dates to bottom
        if (!bDateObj) return -1;
